...
namespace Omise.Example {
    ...
    public abstract class SiteExamples : Example {
        [Test]
        ...
        [Test]
        public async Task LinksCreateCreateForMulti() {
            ...
            var link = await Client.Links.Create(request);
            Assert.AreEqual(99900, link.Amount);
        }

        [Test]
        public async Task LinksCreateCreateForOnce() {
            var request = new CreateLinkRequest {
                Amount = 99900,
                Currency = "thb",
                Title = "Hot Latte",
        ...
        [Test]
        public async Task LinksListList() {
            var links = await Client.Links.GetList();

            var link = links.First(t => t.Id == "link_test_55s7xparb9djkecqkd4");
            Assert.AreEqual(99900, link.Amount);
        }

        [Test]
        public async Task LinksRetrieveRetrieve() {
            var link = await Client.Links.Get("link_test_55s7xparb9djkecqkd4");
            Assert.AreEqual(99900, link.Amount);
        }
    }
}